...
glTF assets use <<uri,URIs>> or <<iri,IRIs>> to reference buffers, files, image resources. Assets **MAY** contain at least these two URI types:
...
Optional _names_ are used for application-specific uses such as display. Any top-level glTF object **MAY** have a `name` string property for this purpose. These property values are not guaranteed to be unique as they are intended to contain values created when the asset was authored.

Optional file-unique IDs (UIDs) **MAY** be added to a top-level glTF object with the `uid` string property. If present, these **MUST** be unique throughout the entire file, and **MUST NOT** share a value with any other `uid` or `name` in the file, except that the same object **MAY** have the same string for both `name` and `uid`. Unique IDs **MUST NOT** contain the following characters: `"`, `#`, `.`, `:`, `@`, `{`, `}`, `[`, `]`, `/`, literal backslashes, or any control characters.

Whereas indices are used for internal glTF references, unique IDs may be used for externally referencing objects in glTF files in a stable and unambiguous way. Such external references are exact case-sensitive matches. Additionally, if the `name` property conforms to all of the requirements of UIDs (uniqueness and character set), then the `name` property **MAY** be used as a fallback when the requested `uid` does not exist in the file. This allows glTF 2.0 files to have their objects referred to externally, as well as glTF 2.1+ files generated without UIDs.
...
As of glTF 2.1, the ability to define more than one scene is deprecated. The `scenes` array **SHOULD** contain at most one scene, and if the `scene` property is defined, it **SHOULD** be set to `0`. Client implementations **SHOULD** use the `scene` property when importing glTF 2.x assets, and **MUST NOT** generate more than one scene or a `scene` property set to a value other than `0` when exporting glTF 2.1 assets.
...
[options="header"]
|====
| `componentType` | Data Type | Simple Name | Signed | Bits
| `5120` | _signed byte_ | `int8` | Signed, two's complement | 8
| `5121` | _unsigned byte_ | `uint8` | Unsigned | 8
| `5122` | _signed short_ | `int16` | Signed, two's complement | 16
| `5123` | _unsigned short_ | `uint16` | Unsigned | 16
| `5124` | _signed int_ | `int32` | Signed, two's complement | 32
| `5125` | _unsigned int_ | `uint32` | Unsigned | 32
| `5126` | _float_ | `float32` | Signed | 32
| `5130` | _double_ | `float64` | Signed | 64
| `5131` | _half float_ | `float16` | Signed | 16
| `5134` | _signed int64_ | `int64` | Signed, two's complement | 64
| `5135` | _unsigned int64_ | `uint64` | Unsigned | 64
|====

The core glTF 2.1 specification defines all of the above component types. However, only a subset of these types are used in the core specification. Individual properties that reference accessors will define which component types are valid for that property, such as the table below for <<mesh-primitive-attributes,Mesh Primitive Attributes>>.

Floating-point data **MUST** use <<ieee-754,IEEE-754>> formats.
...
Signed integer components, unsigned integer sizes beyond 16 bits, and float sizes other than 32-bit are not supported for vertex attributes in the core glTF 2.1 specification. Extensions or future specification versions **MAY** allow additional component types.
...
`TEXCOORD_n`, `COLOR_n`, `JOINTS_n`, and `WEIGHTS_n` attribute semantic property names **MUST** be of the form `[semantic]_[set_index]`, e.g., `TEXCOORD_0`, `TEXCOORD_1`, `COLOR_0`. Indices for indexed attribute semantics **SHOULD** start with 0 and be consecutive positive integers: `TEXCOORD_0`, `TEXCOORD_1`, etc. Indices **MUST NOT** use leading zeroes to pad the number of digits (e.g., `TEXCOORD_01` is not allowed). `TEXCOORD_0` **SHOULD** be used for the primary texture coordinate set (usually the one used for the albedo texture, if any).
...
`indices` accessor **MUST NOT** use signed integer or floating-point component types. `indices` accessor **MUST** use one of _unsigned byte_, _unsigned short_, or _unsigned int_ component types in the core glTF 2.1 specification.
...
Integer sizes beyond 16 bits and float sizes other than 32-bit are not supported for morph attributes in the core glTF 2.1 specification. Extensions or future specification versions **MAY** allow additional component types.
...
Signed integer components, unsigned integer sizes beyond 16 bits, and float sizes other than 32-bit are not supported for joint indices and weights in the core glTF 2.1 specification. Extensions or future specification versions **MAY** allow additional component types.
...
The `mimeType` property is required and **MUST** be defined for glTF 2.1 files. However, for compatibility with glTF 2.0, client implementations **MUST** also support the case when `mimeType` is undefined for images using the `uri` property.
...
Client implementations **MAY** need to manually determine the media type of some images when reading glTF 2.0 files. In such a case, the following table **SHOULD** be used to check the values of the first few bytes.
...
The image data **MUST** match the `image.mimeType` property.
...
glTF 2.1 defines the ability to compose multiple glTF files together, by referencing external glTF files from another glTF file. Any glTF file which references another glTF file as an external asset is called a complex scene.

Files may be added to a top-level `files` array. The file's data may either be stored externally and referenced by a `uri`, stored internally in a base64-encoded data URI, or stored internally in a buffer view and referenced by `bufferView` index. Each file object **MUST** define a `mimeType` property, which may be any media type that matches the file's contents. If the file contains URIs, and the file is embedded data (base64 data URI or bufferView), those URIs are matched against other objects in the `files` array by `name`. Cyclical references between files are strictly prohibited.
...
Nodes may contain an `externalAsset` property that refers to an external glTF asset in the `externalAssets` array by index. This instantiates the external asset as this node in the scene hierarchy, such that the root of the file becomes this node. For vanilla glTF, this means that the scene object in the `scenes` array becomes this node, and the root nodes in the scene object of the nested glTF become children of this node. Extensions or future spec versions **MAY** alter this behavior. For glTF, only files with one scene and `"scene": 0` are valid targets.

Extensions **MAY** be used on objects in the `externalAssets` array, allowing customizations to be shared between multiple instances of external glTF assets on nodes.
...
The following extensions were optional in previous glTF versions, but are now required in glTF 2.1:
...
glTF implementations **MUST** support these extensions in order to be compliant with glTF 2.1.
...
* `length` is the total length of the Binary glTF, including _header_ and all _chunks_, in bytes. In binary format version 3, the most significant bit **MUST** be 0, which limits file sizes to 8 EiB (exbibytes).
...
In binary format version 2, the start and the end of each chunk **MUST** be aligned to a 4-byte boundary. In binary format version 3, the start of each chunk **MUST** be aligned to an 8-byte boundary, and the end of plainly encoded chunks **MUST** be aligned to an 8-byte boundary. The start of each chunk is the next byte-aligned boundary after the end of the previous chunk. See chunks definitions for padding schemes. Chunks **MUST** appear in exactly the order given in <<table-chunktypes>>.
...
| 2. | 0x004E4942 | BIN\0 | Binary buffer           | 0 or 1
|====
...
When plainly encoded, this chunk **MUST** be padded with trailing `Space` chars (`0x20`) to satisfy alignment requirements. When using a different encoding method, the padding scheme **MAY** be defined by the encoding method.
...
When plainly encoded, this chunk **MUST** be padded with trailing zeros (`0x00`) to satisfy alignment requirements. When using a different encoding method, the padding scheme **MAY** be defined by the encoding method.
